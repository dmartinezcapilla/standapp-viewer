    ...
    Object.defineProperties(RandomizeFacilitatorView.prototype,
    {
        init : {
            value: function()
            {
                ...
                $("#start_randomize_facilitator").off("click").on("click", function()
                {
        ...
        load : {
            value: function(data)
            {
                ...
                var segments = [];
                var colors = ['#1abc9c', '#f1c40f','#f39c12', '#27ae50', '#3498db','#e74c3c', '#9b59b6', '#c0392b', '#d35400', "#2980b9"];
                ...
                $.each( data.issues, function( key, issue )
                {
                    ...
                    if (issue.fields.assignee && issue.fields.assignee.name)
                    {
                        ...
                        {
                            self.usersMap.set(displayName, issue.fields.assignee.avatarUrls["48x48"]);
                            segments.push({'fillStyle' : colors[(segments.length-1) % 10], 'text' : displayName});
                        }
        ...
        alertPrize : {
            value: function()
            {
                ...
                $("#winner-image").html("");
                $("<img/>", {src: self.usersMap.get(winningSegment.text), height: "100px", width: "100px"}).appendTo($("#winner-image"));
                $("#winner-text").html(winningSegment.text + ", <br> today is your day!");
