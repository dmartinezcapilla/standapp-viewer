    ...
    load: {
      value: function (data) {
        ...
        $.each(data.issues, function (key, issue) {
          var issueTypeUserStory =
            g_issuetype_map.user_story[issue.fields.issuetype.id];

          //console.log("Issue assignees: ", issue.fields.assignee);
          if (
            issue.fields.assignee &&
            issue.fields.assignee.name == g_evaluatedUser
          ) {
            $.each(issue.fields.worklog.worklogs, function (key2, worklog) {
              if (worklog.author.name == g_evaluatedUser) {
                ...
                snw_supportAnalyzer[worklog.author.name] +=
          ...
          if (issueTypeUserStory != undefined) {
            ...
            storyInitialRowValues[US_TYPE_ICON] =
              '<img class="issuetype" src="' +
              issue.fields.issuetype.iconUrl +
              '"/>';
            storyInitialRowValues[US_STATUS] = "";
            storyInitialRowValues[US_STATUS_ICON] =
              '<img class="statusicon" src="' +
            ...
            if (storyMetricsMap.get(parentId) == undefined) {
              ...
              if (self.getSubtaskType(subtask) != "finding") {
                ...
                storyMetricsMap.get("Summary")[remTimeColIndex] +=
                  timeRemaining;
                ...
                storyMetricsMap.get(parentId)[TEST_TIME_LOGGED] +=
                  timeLogged / 2;
                storyMetricsMap.get(parentId)[DEV_TIME_REMAINING] +=
                  timeRemaining / 2;
                storyMetricsMap.get(parentId)[TEST_TIME_REMAINING] +=
                  timeRemaining / 2;
        ...
        snw_supportAnalyzer["%support"] =
          (snw_supportAnalyzer["totalOthers"] /
            (snw_supportAnalyzer["totalUser"] +
              snw_supportAnalyzer["totalOthers"])) *
          100;
      ...
      value: function (value, key, map) {
        ...
        if (key != "Summary") {
          rowData = $("<tr/>", {
            ...
            class: "story",
            html:
              "<td class='mdl-data-table__cell--non-numeric non-export-field'> " +
              value[US_TYPE_ICON] +
              "</td>" +
              ...
              value[US_USP] +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringHours(value[DEV_TIME_LOGGED]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringHours(value[DEV_TIME_ESTIMATED]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric sos-field'> " +
              this.formatToStringHours(value[DEV_TIME_REMAINING]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringHours(value[AT_TIME_LOGGED]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringHours(value[AT_TIME_ESTIMATED]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric sos-field'> " +
              this.formatToStringHours(value[AT_TIME_REMAINING]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringHours(value[TEST_TIME_LOGGED]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringHours(value[TEST_TIME_ESTIMATED]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric sos-field'> " +
              this.formatToStringHours(value[TEST_TIME_REMAINING]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringPercentage(value[DEV_DEVIATION]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringPercentage(value[AT_DEVIATION]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringPercentage(value[TEST_DEVIATION]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric eos-field'> " +
              this.formatToStringHours(value[FINDINGS_LOGGED]) +
              "</td>" +
              "<td class='mdl-data-table__cell--non-numeric non-export-field'> " +
              value[US_STATUS_ICON] +
              "</td>",
          });
    ...
    paintSOSStorySummaryRow: {
      value: function (map) {
        ...
        var rowData = $("<tr/>", {
          ...
          class: "summary-row",
          html:
            "<td class='mdl-data-table__cell--non-numeric '> TOTALS: </td>" +
